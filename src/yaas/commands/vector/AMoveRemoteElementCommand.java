...
public class AMoveRemoteElementCommand<ElementType> implements Command{
	...
	public AMoveRemoteElementCommand(ListenableVector<ElementType> buf, int aFromPosition, ListenableVector<ElementType> aRemoteBuffer, int pos) {
		buffer = buf;
		toPosition = pos;
...
//		oldVal = remoteBuffer.get(toPosition);
		buffer.move(fromPosition, remoteBuffer, toPosition);
		return null;
	}
	public void undo() {
		remoteBuffer.move(toPosition, buffer, fromPosition);
	}
